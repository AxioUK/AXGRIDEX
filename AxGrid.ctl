...
Begin VB.UserControl AxGrid 
   ...
   Begin VB.TextBox txtInfoBar 
      ...
      Width           =   5865
   End
   Begin AXGRIDEX.axComboBox cBox 
      Height          =   255
      Left            =   2670
      TabIndex        =   5
      Top             =   2655
...
'|
'| AXGRID UserControl
'| v2.3.1 with ADO Features
'| Dependencies: MsFlexGrid, MsADO 2.8
'| Author: AxioUK
'| Date  : 17-MAR-2014
'| Lates Release : 08-JUN-2016
'|--------------------------------------------
Option Explicit
...
Attribute AboutBox.VB_MemberFlags = "40"
  ...
  With AxGridAbout
      ...
      .lblVersion.Caption = "v" & App.Major & "." & App.Minor & "." & App.Revision & "ADO"
      .lblV1.Caption = App.Major
      .lblV2.Caption = "." & App.Minor
      .lblInfo.Caption = "Another Xtended Grid with ADO Features"
      .Show (vbModal)
  End With
End Sub
...
Public Function ADOCreateGridTable(sTableName As String, Optional bSaveOptions As Boolean) As Boolean
Dim strFields As String, sFields As String
Dim sValues As Variant
Dim ic As Long, ir As Long

  For ic = 0 To fg.Cols - 1
    If fg.TextMatrix(0, ic) <> "" Or fg.TextMatrix(0, ic) <> vbNullString Then
      If ic < fg.Cols - 1 Then
        strFields = strFields & fg.TextMatrix(0, ic) & " char(60), "
      ElseIf ic = fg.Cols - 1 Then
        strFields = strFields & fg.TextMatrix(0, ic) & " char(60)"
      End If
      sFields = sFields & fg.TextMatrix(0, ic) & ", "
    Else
      If ic < fg.Cols - 1 Then
        strFields = strFields & "COL" & ic & " char(60), "
      ElseIf ic = fg.Cols - 1 Then
        strFields = strFields & "COL" & ic & " char(60)"
      End If
      sFields = sFields & "COL" & ic & ", "
    End If
  Next ic
...
End If
If Right$(sFields, 2) = ", " Then
  sFields = Mid$(sFields, 1, Len(sFields) - 2)
End If

'On Error GoTo adoErrS
'--<Crear TABLA>----------------------------
m_Command = "CREATE TABLE " & sTableName & " (" & strFields & ");"
Debug.Print m_Command
Set Cnn = New ADODB.Connection
Cnn.Open m_Connection
Cnn.Execute m_Command

'---<Insertar Filas a TABLA>----------------
For ir = 1 To fg.Rows - 1
  sValues = ""
  For ic = 0 To fg.Cols - 1
    sValues = sValues & "'" & fg.TextMatrix(ir, ic) & "', "
  Next ic
  If Right$(sValues, 2) = ", " Then
  sValues = Mid$(sValues, 1, Len(sValues) - 2)
  End If
  m_Command = "INSERT INTO " & sTableName & " (" & sFields & ") VALUES (" & sValues & ")"
  Debug.Print m_Command
  Cnn.Execute m_Command
Next ir

ADOCreateGridTable = True
Set Cnn = Nothing

If Not IsMissing(bSaveOptions) Then
  If bSaveOptions Then
      m_Fields = sFields
      PropertyChanged "ADOFields"
      m_Table = sTableName
      PropertyChanged "ADOTable"
  End If
End If
...
adoErrS:
Set Cnn = Nothing
ADOCreateGridTable = False
End Function

Public Function ADOCreateTable(sTableName As String, Optional bSaveOptions As Boolean, Optional iniCol As Long, Optional lastCol As Long) As Boolean
Dim stFields As String
Dim i As Long

  For i = iniCol To lastCol
    If fg.TextMatrix(0, i) <> "" Or fg.TextMatrix(0, i) <> vbNullString Then
      If i < fg.Cols - 1 Then
        stFields = stFields & fg.TextMatrix(0, i) & " char(60), "
      ElseIf i = fg.Cols - 1 Then
        stFields = stFields & fg.TextMatrix(0, i) & " char(60)"
      End If
    Else
      If i < fg.Cols - 1 Then
        stFields = stFields & "COL" & i & " char(60), "
      ElseIf i = fg.Cols - 1 Then
        stFields = stFields & "COL" & i & " char(60)"
      End If
    End If
...
If Right$(stFields, 2) = ", " Then
  stFields = Mid$(stFields, 1, Len(stFields) - 2)
End If

m_Command = "CREATE TABLE " & sTableName & " (" & stFields & ");"
...
Public Function ADODropTable(sTableName As String) As Boolean

m_Command = "DROP TABLE " & sTableName & ";"
...
Public Function ADOInsertRow(lRow As Long, Optional iniCol As Long, Optional lastCol As Long) As Boolean
Dim sValues As Variant
Dim i As Long
Dim iCol As Long, lCol As Long

  If Not IsMissing(iniCol) Then
    iCol = iniCol
  Else
    iCol = 0
  End If
  If Not IsMissing(lastCol) Then
    lCol = lastCol
  Else
    lCol = fg.Cols - 1
  End If
...
On Error GoTo adoErrS
For i = iCol To lCol
  sValues = sValues & "'" & fg.TextMatrix(lRow, i) & "', "
Next i
...
m_Command = "INSERT INTO " & m_Table & " (" & m_Fields & ") VALUES(" & sValues & ")"
Debug.Print m_Command
...
Public Function ADOInsertSelect(sTableInsert As String, sFieldsInsert As String, _
                                sTableSelect As String, sFieldsSelect As String, _
                                Optional adoWhereField As String, Optional adoLikeValue As Variant) As Boolean
'INSERT INTO "table1" ("column1", "column2") SELECT "column3", "column4" FROM "table2";
On Error GoTo adoErrS
If adoWhereField = "" Or adoWhereField = vbNullString Then
  m_Command = "INSERT INTO " & sTableInsert & " (" & sFieldsInsert & ") SELECT " & sFieldsSelect & " FROM " & sTableSelect & ";"
Else
  If Not IsMissing(adoLikeValue) Then
    If adoLikeValue <> "" Or adoLikeValue <> vbNullString Then
      m_Command = "INSERT INTO " & sTableInsert & " (" & sFieldsInsert & ") SELECT " & sFieldsSelect & " FROM " & sTableSelect & " WHERE " & adoWhereField & " LIKE '%" & adoLikeValue & "%'"
    Else
      MsgBox "Debe indicar un valor para '" & adoWhereField & "'"
    End If
  Else
    MsgBox "Debe indicar un valor para '" & adoWhereField & "'"
  End If
...
Cnn.Open m_Connection
Cnn.Execute m_Command
ADOInsertSelect = True
Set Cnn = Nothing

Exit Function
adoErrS:
Set Cnn = Nothing
ADOInsertSelect = False
End Function

Public Function ADOSelect(withConditions As Boolean, Optional adoWhereField As String, Optional adoLikeValue As Variant) As Boolean
On Error GoTo adoErrS

If withConditions = False Then
  m_Command = "SELECT " & m_Fields & " FROM " & m_Table
  GoTo Cargar
End If
If withConditions And Not IsMissing(adoWhereField) And Not IsMissing(adoLikeValue) Then
    If adoWhereField <> vbNullString And adoLikeValue <> vbNullString Then
      If adoWhereField <> "" And adoLikeValue <> "" Then
        m_Command = "SELECT " & m_Fields & " FROM " & m_Table & " WHERE " & adoWhereField & " LIKE '%" & adoLikeValue & "%'"
        GoTo Cargar
      Else
        GoTo adoErrS
      End If
...
Cargar:
  LoadFlex m_Command, m_Connection, fg, True, False
  ADOSelect = True

Exit Function
adoErrS:
ADOSelect = False
MsgBox "Debe indicar un campo y valor válidos!"
End Function
...
Public Sub LoadfromDB(strSQL_Consulta As String, Optional strADOConnection As String, Optional bNumberedFixedCol As Boolean, Optional bAutoSizeCols As Boolean)
Dim bNum As Boolean, bAsize As Boolean

If strADOConnection = vbNullString Then
  If m_Connection = vbNullString Then
    MsgBox "Debe especificar String de Conexión", vbExclamation + vbOKOnly, "Error de acceso a Tabla"
  Else
    strADOConnection = m_Connection
  End If
End If

If IsMissing(bNumberedFixedCol) Then
  bNum = False
Else
  bNum = bNumberedFixedCol
End If
If IsMissing(bAutoSizeCols) Then
  bAsize = False
Else
  bAsize = bAutoSizeCols
End If

LoadFlex strSQL_Consulta, strADOConnection, fg, bNum, bAsize
End Sub

Public Sub LoadfromRST(rRST As ADODB.Recordset, Optional bNumberedFixedCol As Boolean, Optional bAutoSizeCols As Boolean)
Dim bNum As Boolean, bAsize As Boolean
...
LoadRST rRST, fg, bNum, bAsize
End Sub

Public Sub LoadToObject(objType As eTypeControl, strSQLQuery As String, strField As String, Optional strADOConnection As String)
On Error GoTo ErrorHandler
  ...
  Rst.Open strSQLQuery, Cnn, adOpenDynamic, adLockOptimistic
    If objType = oComboBox Then Call cBox.LoadfromRST(Rst, strField)
    If objType = oListBox Then Call LoadListBox(LBox, Rst, strField)
  Rst.Close
  Cnn.Close
...
Public Function SaveAsExcel(sFilename As String, sSheetName As String, bAppendExistXLS As Boolean)
  AppendExcelSheet fg, sFilename, sSheetName, bAppendExistXLS
End Function
...
Private Function AppendExcelSheet(rsSrc As MSFlexGrid, sXlsFile As String, Optional ByVal sSheetName As String, _
    ...
    With New ADOX.Catalog
        ...
        For C = 0 To rsSrc.Cols - 1
            Set oCol = New ADOX.Column
            With oCol
                If rsSrc.TextMatrix(0, C) = "" Then
                  .Name = "UnNamedCol" & C
                Else
                  .Name = rsSrc.TextMatrix(0, C)
                End If
                .Type = adVarWChar
            End With
...
'****************************
Private Sub LoadFlex(m_RecordSet As String, strConnection As String, flexGrid As MSFlexGrid, NumberedFixedCol As Boolean, AutoSizeColumns As Boolean)
On Local Error GoTo Error_Handler
    ...
    Rst.Open m_RecordSet, Cnn, adOpenDynamic, adLockOptimistic
    ' -- Configurar el Grid
    With flexGrid
      ' -- limpiamos el grid
      .Clear
      ...
      ' -- Recorrer los campos del recordset
          For Columna = 0 To Rst.Fields.Count - 1
            ...
            .TextMatrix(0, Columna + 1) = Rst.Fields(Columna).Name
          Next Columna
          Fila = 1
          ...
          Do While Not Rst.EOF
            ...
            'Si TRUE Autonumerar Columna Fija
            If NumberedFixedCol = True Then
              .ColAlignment(0) = 3
              .TextMatrix(0, 0) = "N"
              .TextMatrix(Fila, 0) = Str(Fila)
            End If
            ...
            For Columna = 0 To Rst.Fields.Count - 1
                ...
                If Not IsNull(Rst.Fields(Columna).Value) Then
                    ...
                    .TextMatrix(Fila, Columna + 1) = Rst.Fields(Columna).Value
                End If
            Next
            ' -- Siguiente registro
            Rst.MoveNext
      ...
      If AutoSizeColumns = True Then
        
        Call AutoSizeCols(0, fg.Cols - 1)
      End If
...
Private Sub LoadRST(sqlRST As ADODB.Recordset, flexGrid As MSFlexGrid, NumberedFixedCol As Boolean, AutoSizeColumns As Boolean)
On Local Error GoTo Error_Handler
    ' -- Variables para las filas y columnas del Flexgrid
    Dim Columna As Long, Fila As Long
    ...
    With flexGrid
      ...
      ' -- Cantidad de filas y columnas
      .Rows = 1
      .Cols = sqlRST.Fields.Count + 1
      ' -- Recorrer los campos del recordset
          For Columna = 0 To sqlRST.Fields.Count - 1
            ' -- Añade el título del campo al encabezado de columna
            .TextMatrix(0, Columna + 1) = sqlRST.Fields(Columna).Name
          Next Columna
          Fila = 1
          ...
          Do While Not sqlRST.EOF
            ...
            For Columna = 0 To sqlRST.Fields.Count - 1
                ...
                If Not IsNull(sqlRST.Fields(Columna).Value) Then
                    ' -- Agrega el registro en la fila y columna específica
                    .TextMatrix(Fila, Columna + 1) = sqlRST.Fields(Columna).Value
                End If
            Next
            ' -- Siguiente registro
            sqlRST.MoveNext
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    m_Connection = PropBag.ReadProperty("ADOConnection", vbNullString)
    m_Fields = PropBag.ReadProperty("ADOFields", vbNullString)
    m_Table = PropBag.ReadProperty("ADOTable", vbNullString)
    ' Mapped Properties
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("ADOConnection", m_Connection, vbNullString)
    Call PropBag.WriteProperty("ADOFields", m_Fields, vbNullString)
    Call PropBag.WriteProperty("ADOTable", m_Table, vbNullString)
    ' Mapped Properties
...
Public Property Let ADOConnection(New_Connection As String)
  m_Connection = New_Connection
  PropertyChanged "ADOConnection"
End Property

Public Property Let ADOFields(New_Fields As String)
  m_Fields = New_Fields
  PropertyChanged "ADOFields"
End Property

Public Property Let ADOTable(New_Table As String)
  m_Table = New_Table
  PropertyChanged "ADOTable"
End Property
