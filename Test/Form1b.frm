...
Begin VB.Form Form1 
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   315
      Width           =   8835
      Begin AXGRIDEX.AxBiGrid AxBiGrid1 
         Height          =   3810
         Left            =   90
         TabIndex        =   78
         Top             =   210
...
Private Sub AxBiGrid1_CellTextChange(xGrid As AXGRIDEX.eSideGrid, ByVal Row As Long, ByVal Col As Long)
If AxBiGrid1.ColObject(eLeftGrid, 2) = eTextBoxColumn Then
  AxBiGrid1.SetColObject(eLeftGrid, 2, True) = eComboBoxColumn
End If
End Sub

Private Sub AxBiGrid1_Click(xGrid As AXGRIDEX.eSideGrid)
...
Select Case Index
   ...
   Case 1
    AxBiGrid1.LoadfromRST Rst, eRightGrid, False, True
...
SQLString = "SELECT EQ.EQID, EQ.SERIE, SEN.GAS, GR.GNOMBRE FROM SENSOR AS SEN INNER JOIN (GRUPO AS GR INNER JOIN (EMPRESA AS EM INNER JOIN " & _
            "(AREA AS AR INNER JOIN EQUIPOS AS EQ ON AR.AID = EQ.AREA) ON EM.EID = EQ.EMPRESA) ON GR.GID = EQ.GRUPO) ON SEN.SID = EQ.SENSOR " & _
            "WHERE GR.GNOMBRE='FLSMIDTH' ORDER BY EQ.EQID;"
  ...
  AxBiGrid1.LoadfromRST Rst, eLeftGrid
  Rst.Close
...
Private Sub Command5_Click(Index As Integer)
   AxGrid1.LoadToObject oComboBox, "SELECT * FROM Unidad", "MEDIDA", Conn
End Sub
...
Select Case Index
  ...
  Case 3
     AxBiGrid1.LoadfromDB eRightGrid, Consulta, Conn, True
