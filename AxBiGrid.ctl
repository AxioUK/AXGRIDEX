...
'Mapped Events
Public Event Click(xGrid As eSideGrid)
Public Event Compare(ByVal Row1 As Long, ByVal Row2 As Long, Cmp As Integer)
Public Event DblClick(xGrid As eSideGrid)
...
Attribute AboutBox.VB_MemberFlags = "40"
  ...
  With AxGridAbout
      ...
      .lblInfo.Caption = "Dual version of AxGrid[ADO]: Most Enhanced yet!"
...
Public Function ADOSelect(eSide As eSideGrid, bAutoSizeCols As Boolean, withConditions As Boolean, Optional adoWhereField As String, Optional adoLikeValue As Variant) As Boolean
...
If withConditions And Not IsMissing(adoWhereField) And Not IsMissing(adoLikeValue) Then
    If adoWhereField <> vbNullString And adoLikeValue <> vbNullString Then
      If adoWhereField <> "" And adoLikeValue <> "" Then
        If Not IsMissing(m_Fields) And Not m_Fields = vbNullString And Not m_Fields = "" Then
...
If eSide = eLeftGrid Then
  If m_Command <> "" Then LoadFlex m_Command, m_Connection, eLeftGrid, True, bAutoSizeCols
Else
  If m_Command <> "" Then LoadFlex m_Command, m_Connection, eRightGrid, False, bAutoSizeCols
End If
  ADOSelect = True
...
Public Sub LoadfromDB(eSide As eSideGrid, strSQL_Consulta As String, Optional strADOConnection As String, Optional bNumberedFixedCol As Boolean, Optional bAutoSizeCols As Boolean)
Dim bNum As Boolean, bAsize As Boolean

If IsMissing(strADOConnection) Or strADOConnection = vbNullString Or strADOConnection = "" Then
  If IsMissing(m_Connection) Or m_Connection = vbNullString Or m_Connection = "" Then
...
LoadFlex strSQL_Consulta, strADOConnection, eSide, bNum, bAsize
...
Public Sub LoadfromRST(rRST As ADODB.Recordset, eSide As eSideGrid, Optional bNumberedFixedCol As Boolean, Optional bAutoSizeCols As Boolean)
Dim bNum As Boolean, bAsize As Boolean
...
LoadRST rRST, eSide, bNum, bAsize
...
'On Error GoTo ErrorHandler
  ...
  Cnn.Open strADOConnection
    Rst.Open strSQLQuery, Cnn, adOpenDynamic, adLockOptimistic
      If objType = oComboBox Then
         Call cBox.LoadfromRST(Rst, strField)
      ElseIf objType = oListBox Then
         Call LoadListBox(LBox, Rst, strField)
      End If
    Rst.Close
  Cnn.Close
...
Public Function SaveAsExcel(sFilename As String)
    ...
    Dim lRow As Integer, lCol As Integer, lCol2 As Integer
...
Private Sub LoadFlex(m_RecordSet As String, strConnection As String, eSide As eSideGrid, NumberedFixedCol As Boolean, AutoSizeColumns As Boolean)
On Error GoTo Error_Handler
    ...
    With fg(iDX)
      ...
      ' -- Recorrer los campos del recordset
          ...
          Do While Not Rst.EOF
            ...
            For Columna = 0 To Rst.Fields.Count - 1
                ' -- Comprobar que el valor no es nulo
        ...
        If AutoSizeColumns = True Then
            Call AutoSizeCols(eSide, 0, .Cols - 1)
        End If
    ...
    If m_BackColorAlternate <> m_BackColor Then
       Call SetAlternateRowColors(CLng(iDX), m_BackColorAlternate, m_BackColor)
    Else
       fg(iDX).BackColor = m_BackColor
    End If
...
Private Sub LoadRST(sqlRST As ADODB.Recordset, eSide As eSideGrid, NumberedFixedCol As Boolean, AutoSizeColumns As Boolean)
    ...
    With fg(iDX)
      ...
      ' -- Recorrer los campos del recordset
          For Columna = 0 To sqlRST.Fields.Count - 1
            ' -- Añade el título del campo al encabezado de columna
            .TextMatrix(0, Columna) = sqlRST.Fields(Columna).Name
          Next Columna
          Fila = 1
          ...
          Do While Not sqlRST.EOF
            ...
            For Columna = 0 To sqlRST.Fields.Count - 1
                ...
                If Not IsNull(sqlRST.Fields(Columna).Value) Then
                    ' -- Agrega el registro en la fila y columna específica
                    .TextMatrix(Fila, Columna) = sqlRST.Fields(Columna).Value
                End If
            Next
            ' -- Siguiente registro
            sqlRST.MoveNext
...
'Public Property Get ADOConnection() As String
...
'Public Property Get ADOFields() As String
'  ADOFields = m_Fields
'End Property
...
'Public Property Get ADOTable() As String
'  ADOTable = m_Table
'End Property
    ...
    With fg(iDX)
         For i = eRowInitial To eRowFinal
            Select Case Settings
                Case Is = axSTSum
                     nValue = nValue + Val(.TextMatrix(i, eColumn))
                Case Is = axSTMax
                     If bFirst Then
                        nValue = Val(.TextMatrix(i, eColumn))
                        bFirst = False
                     Else
                        nValue = Max(nValue, Val(.TextMatrix(i, eColumn)))
                     End If
                Case Is = axSTMin
                     If bFirst Then
                        nValue = Val(.TextMatrix(i, eColumn))
                        bFirst = False
                     Else
                        nValue = Min(nValue, Val(.TextMatrix(i, eColumn)))
                     End If
                Case Is = axSTCount
         ...
         If Settings = axSTMultiply Then
            nValue = Val(.TextMatrix(eRowInitial, eColumn)) * Val(.TextMatrix(eRowFinal, eColumn))
         End If
    End With
    ...
    With fg(iDX)
         For i = eRowInitial To eRowFinal
             For j = eColumnInitial To eColumnFinal
                 Select Case Settings
                        Case Is = axSTSum
                              nValue = nValue + Val(.TextMatrix(i, j))
                        Case Is = axSTMax
                             If bFirst Then
                                nValue = Val(.TextMatrix(i, j))
                                bFirst = False
                             Else
                                nValue = Max(nValue, Val(.TextMatrix(i, j)))
                             End If
                        Case Is = axSTMin
                             If bFirst Then
                                nValue = Val(.TextMatrix(i, j))
                                bFirst = False
                             Else
                                nValue = Min(nValue, Val(.TextMatrix(i, j)))
                             End If
                        Case Is = axSTCount
...
Public Property Get CalculateRow(eSide As eSideGrid, Settings As eSubTotalSettings, eRow As Long, _
    ...
    With fg(iDX)
         For i = eColInitial To eColFinal
            Select Case Settings
                Case Is = axSTSum
                     nValue = nValue + Val(.TextMatrix(eRow, i))
                Case Is = axSTMax
                     If bFirst Then
                        nValue = Val(.TextMatrix(eRow, i))
                        bFirst = False
                     Else
                        nValue = Max(nValue, Val(.TextMatrix(eRow, i)))
                     End If
                Case Is = axSTMin
                     If bFirst Then
                        nValue = Val(.TextMatrix(eRow, i))
                        bFirst = False
                     Else
                        nValue = Min(nValue, Val(.TextMatrix(eRow, i)))
                     End If
                Case Is = axSTCount
         ...
         If Settings = axSTMultiply Then
            nValue = Val(.TextMatrix(eRow, eColInitial)) * Val(.TextMatrix(eRow, eColFinal))
         End If
    End With
...
Public Property Get ValueMatrix(eG As eSideGrid, ByVal Row As Long, ByVal Col As Long) As Variant
     ValueMatrix = Val(fg(eG).TextMatrix(Row, Col))
End Property
